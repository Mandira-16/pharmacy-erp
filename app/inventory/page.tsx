...
export default function InventoryPage() {
  ...
  const fetchInventory = () => {
    setLoading(true)
    fetch('/api/inventory').then(r => r.json()).then(d => { if (d.error) throw new Error(d.error); setMedicines(d.medicines ?? []); setKpis(d.kpis) }).catch(e => setError(e.message)).finally(() => setLoading(false))
  }
  ...
  if (status === 'loading' || loading) return <AppLayout><div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100vh', flexDirection: 'column', gap: '12px' }}><div style={{ width: '28px', height: '28px', border: '3px solid var(--border-default)', borderTopColor: 'var(--brand-primary)', borderRadius: '50%', animation: 'spin 0.7s linear infinite' }} /><span style={{ fontSize: '13px', color: 'var(--text-tertiary)' }}>Loading inventory...</span></div></AppLayout>
    ...
    <AppLayout>
      <div style={{ padding: '28px 32px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '28px' }}>
          ...
          <button onClick={fetchInventory} style={{ fontSize: '12px', padding: '8px 16px', background: 'var(--bg-surface)', border: '1px solid var(--border-default)', borderRadius: 'var(--radius-md)', color: 'var(--text-secondary)', cursor: 'pointer', fontFamily: 'inherit', fontWeight: '500', boxShadow: 'var(--shadow-sm)' }}>↻ Refresh</button>
        </div>
        ...
        <div style={{ display: 'flex', gap: '10px', marginBottom: '16px', flexWrap: 'wrap' }}>
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search medicine or SKU..." style={{ flex: 1, minWidth: '200px', padding: '9px 14px', background: 'var(--bg-surface)', border: '1px solid var(--border-default)', borderRadius: 'var(--radius-md)', color: 'var(--text-primary)', fontSize: '13px', outline: 'none', boxShadow: 'var(--shadow-sm)' }} />
          ...
          <span style={{ display: 'flex', alignItems: 'center', padding: '9px 14px', background: 'var(--bg-surface)', border: '1px solid var(--border-default)', borderRadius: 'var(--radius-md)', fontSize: '12px', color: 'var(--text-tertiary)', fontWeight: '500' }}>{filtered.length} of {medicines.length}</span>
        </div>
        ...
        <div style={{ background: 'var(--bg-surface)', border: '1px solid var(--border-default)', borderRadius: 'var(--radius-lg)', overflow: 'hidden', boxShadow: 'var(--shadow-sm)' }}>
          <div style={{ padding: '16px 24px', borderBottom: '1px solid var(--border-subtle)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            {kpis && kpis.expiring30 > 0 && <span style={{ fontSize: '12px', background: 'var(--danger-bg)', border: '1px solid var(--danger-border)', color: 'var(--danger)', padding: '4px 12px', borderRadius: '20px', fontWeight: '600' }}>⚠ {kpis.expiring30} expiring within 30 days</span>}
          </div>
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              <tbody>
                {filtered.length === 0 ? (
                  <tr><td colSpan={10} style={{ padding: '40px', textAlign: 'center', color: 'var(--text-tertiary)', fontSize: '13px' }}>No medicines match your filters</td></tr>
                ) : filtered.map(med => {
                  const expiryColor = med.daysToExpiry !== null ? med.daysToExpiry <= 30 ? 'var(--danger)' : med.daysToExpiry <= 60 ? '#f97316' : med.daysToExpiry <= 90 ? 'var(--warning)' : 'var(--text-secondary)' : 'var(--text-secondary)'
                  return (
                    ...
                    >
                      ...
                      <td style={{ padding: '13px 16px', fontSize: '12px', color: expiryColor, whiteSpace: 'nowrap' }}>{med.expiryDate ?? '—'}{med.daysToExpiry !== null && med.daysToExpiry <= 90 && <span style={{ marginLeft: '4px', fontSize: '10px' }}>({med.daysToExpiry}d)</span>}</td>
                      ...
                      <td style={{ padding: '13px 16px', whiteSpace: 'nowrap' }}>
                        <span style={{ padding: '3px 10px', borderRadius: '20px', fontSize: '11px', fontWeight: '600', background: `${statusColors[med.status] ?? '#6b7280'}12`, color: statusColors[med.status] ?? '#6b7280', border: `1px solid ${statusColors[med.status] ?? '#6b7280'}25` }}>{med.status}</span>
                      </td>
                    </tr>
                  )
                })}
