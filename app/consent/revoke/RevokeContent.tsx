...
export default function RevokeContent() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/patients/revoke-consent?token=${token}`)
      .then(r => r.json())
      .then(d => { if (d.error) throw new Error(d.error); setPatientName(d.patientName); setStatus('success') })
        ...
        <div style={{ background: 'white', borderRadius: '20px', padding: '48px 40px', maxWidth: '460px', width: '100%', boxShadow: '0 4px 24px rgba(0,0,0,0.08)', textAlign: 'center' }}>
          ...
          {status === 'success' && (
            <div>
              ...
              <p style={{ fontSize: '14px', color: '#475569', lineHeight: '1.7', marginBottom: '24px' }}>Your medication history at Ceylon Pharmacy has been <strong>locked immediately</strong>. No staff can access your records until you grant consent again.</p>
              <div style={{ background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: '12px', padding: '16px', marginBottom: '24px' }}>
                <p style={{ fontSize: '13px', color: '#15803d', fontWeight: '500', lineHeight: '1.6' }}>✓ Consent revoked successfully<br />✓ Records are now protected<br />✓ PDPA compliant — Ceylon Pharmacy</p>
              </div>
              <p style={{ fontSize: '12px', color: '#94a3b8', lineHeight: '1.6' }}>If you wish to grant access again, visit the pharmacy and ask the pharmacist to send a new consent request.</p>
            </div>
          )}
          ...
          {status === 'error' && (
            <div>
              ...
              <div style={{ background: '#fef2f2', border: '1px solid #fecaca', borderRadius: '12px', padding: '14px' }}>
                <p style={{ fontSize: '12px', color: '#dc2626', lineHeight: '1.6' }}>If you need to revoke access, please visit Ceylon Pharmacy directly.</p>
              </div>
            </div>
          )}
